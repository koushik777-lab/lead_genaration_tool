...
import { useGetLead, useGetLeadNotes, useUpdateLeadStage, useAddLeadNote, getGetLeadQueryKey, getGetLeadNotesQueryKey, getGetCrmPipelineQueryKey } from "@workspace/api-client-react";
...
export default function LeadDetail() {
  ...
  const { data: notes } = useGetLeadNotes(id, {
    query: { enabled: !!id, queryKey: getGetLeadNotesQueryKey(id) },
  });
  ...
  const handleStageChange = (stage: string) => {
    updateStage.mutate({ id, data: { stage: stage as "New Lead" | "Contacted" | "Qualified" | "Proposal Sent" | "Closed Won" | "Closed Lost" } });
  };
          ...
          <div className="bg-card border border-card-border rounded-lg p-4">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              {[
                ...
                { label: "SEO Score", value: lead.seoScore !== null && lead.seoScore !== undefined ? `${lead.seoScore}/100` : null },
                { label: "Performance", value: lead.performanceScore !== null && lead.performanceScore !== undefined ? `${lead.performanceScore}/100` : null },
              ].map(({ label, value }) => (
                value ? (
                  <div key={label}>
        ...
        <div className="bg-card border border-card-border rounded-lg p-4 flex flex-col h-fit">
          ...
          <div className="flex gap-2 mt-auto">
            ...
            <button
              onClick={() => {
                if (noteText.trim()) {
                  addNote.mutate({ id, data: { content: noteText.trim() } });
                }
              }}
