import { useGetCrmPipeline, useUpdateLeadStage, getGetCrmPipelineQueryKey } from "@workspace/api-client-react";
...
export default function CrmBoard() {
  const { data, isLoading } = useGetCrmPipeline({
    query: { queryKey: getGetCrmPipelineQueryKey() },
  });
                    ...
                    stageData.leads.map((lead) => (
                      <div key={lead.id} className="bg-background border border-border rounded-md p-3 space-y-2">
                        ...
                        <select
                          ...
                          onChange={(e) => updateStage.mutate({ id: lead.id, data: { stage: e.target.value as "New Lead" | "Contacted" | "Qualified" | "Proposal Sent" | "Closed Won" | "Closed Lost" } })}
                          className="w-full text-xs px-1.5 py-1 bg-muted border border-border rounded text-muted-foreground focus:outline-none"
                          onClick={(e) => e.stopPropagation()}
                        >
                          {["New Lead", "Contacted", "Qualified", "Proposal Sent", "Closed Won", "Closed Lost"].map((s) => (
