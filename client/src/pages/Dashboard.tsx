import {
  ...
  useGetLeadScoreBreakdown,
  useGetPipelineValues,
  useGetRecentActivity,
  useGetIndustryBreakdown,
} from "@/lib/hooks";
import {
  BarChart,
  Bar,
  ...
  return (
    <div className="bg-card border border-card-border rounded-lg p-4">
      <div className="flex items-center gap-3 mb-3">
        <div
          className={`w-8 h-8 rounded-md flex items-center justify-center ${color ?? "bg-primary/10"}`}
        >
          <Icon
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {statsLoading ? (
          Array.from({ length: 8 }).map((_, i) => (
            <div
              key={i}
              className="bg-card border border-card-border rounded-lg p-4 h-24 animate-pulse"
            />
          ))
        ) : stats ? (
          <>
            <StatCard
              ...
              sub={`Avg. score: ${stats.avgLeadScore}`}
            />
            <StatCard
              icon={Flame}
              label="Hot Leads"
                  ...
                  >
                    {scoreBreakdown.map((entry) => (
                      <Cell
                        ...
                        fill={
                          SCORE_COLORS[
                            entry.category as keyof typeof SCORE_COLORS
                          ] ?? "#6b7280"
                        }
                      />
                    ))}
                  ...
                  <div key={item.category} className="flex items-center gap-2">
                    <span
                      ...
                      style={{
                        background:
                          SCORE_COLORS[
                            item.category as keyof typeof SCORE_COLORS
                          ],
                      }}
                    />
